...
public class MainVerticle extends AbstractVerticle {
  ...
  public void start(Future<Void> startFuture) throws Exception {
    Future<Void> steps = prepareDatabase().compose(
      v -> startHttpServer()
    );
  ...
  private Future<Void> prepareDatabase() {
    ...
    jdbcClient.getConnection(ar -> {
      if (ar.failed()) {
        ...
        SQLConnection conneciton = ar.result();
        conneciton.execute(SQL_CREATE_PAGES_TABLE, create -> {
          conneciton.close();
          if (create.failed()) {
  ...
  private static final String EMPTY_PAGE_MAEKDOWN =
    ...
    jdbcClient.getConnection(car -> {
      if (car.succeeded()) {
        ...
        connection.queryWithParams(SQL_GET_PAGE, new JsonArray().add(page), res -> {
          connection.close();
          if (res.succeeded()) {
            JsonArray row = res.result().getResults()
              .stream()
              .findFirst()
              .orElseGet(() -> new JsonArray().add(-1).add(EMPTY_PAGE_MAEKDOWN));
